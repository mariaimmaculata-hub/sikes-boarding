...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-900 via-blue-700 to-teal-600 flex items-center justify-center p-6 font-sans select-none relative">
        
        <!-- LOGIN CARD CONTAINER -->
        <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 border border-slate-100 flex flex-col justify-between">
            
            <div class="text-center mb-6">
                <span class="text-slate-500 text-xs font-semibold uppercase tracking-widest block mb-1">Selamat Datang</span>
                <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">MASUK KE SISTEM</h1>
            </div>

            <!-- STATUS MESSAGE FROM LARAVEL -->
            <div v-if="status" class="mb-4 text-center text-sm font-semibold text-green-600 bg-green-50 py-2 px-4 rounded-xl border border-green-200">
                {{ status }}
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <!-- SELECT ROLE AREA -->
                <div class="space-y-2">
                    <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider block">PILIH PERAN ANDA:</span>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        >
                            ...
                            <span class="font-semibold text-xs mt-1 block">Petugas Klinik</span>
                        </div>
                        ...
                        >
                            <!-- ShieldCheckIcon -->
                            <ShieldCheckIcon class="w-7 h-7 mb-1" :class="form.role === 'admin' ? 'text-purple-600' : 'text-slate-400'" />
                            <span class="font-semibold text-xs mt-1 block">Admin</span>
                        </div>

                    </div>
                    <InputError class="mt-1" :message="clientErrors.role || form.errors.role" />
                </div>

                <!-- USERNAME / EMAIL FIELD -->
                <div class="space-y-1">
                    <label for="email" class="text-sm font-semibold text-slate-700">Username / Email</label>
                    <input 
                        id="email"
                        type="email"
                        placeholder="maria@sekolah.id"
                        v-model="form.email"
                        class="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-slate-800 placeholder-slate-400 transition"
                ...
                <div class="flex items-center justify-between select-none">
                    ...
                    <Link
                        ...
                        class="text-sm text-blue-600 hover:underline font-semibold transition"
                    >
                        Lupa Password?
                    </Link>
                </div>
                ...
                <div>
                    <button 
                        ...
                        class="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold py-3 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 active:scale-[0.98] disabled:opacity-50 uppercase"
                    >
                        MASUK SEKARANG
                    </button>
                </div>
