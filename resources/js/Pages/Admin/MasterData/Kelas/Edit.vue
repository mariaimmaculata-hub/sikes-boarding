...
// Simulating database lookup based on ID
const form = useForm({
    id: props.id,
    name: props.id === '1' ? 'X RPL 1' : (props.id === '5' ? 'X TKJ 1' : 'XI AK 1'),
    level: props.id === '3' || props.id === '6' ? 'XI' : (props.id === '4' ? 'XII' : 'X'),
    department: props.id <= '4' ? 'RPL' : (props.id <= '7' ? 'TKJ' : 'AK')
});

const submit = () => {
    alert(`Kelas ${form.name} berhasil diperbarui!`);
    window.history.back();
};
</script>
    ...
    <AdminLayout>
        ...
        <div class="space-y-6 max-w-xl">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">Ubah Kelas</h2>
                    <p class="text-xs text-slate-500 font-medium">Perbarui rombel / kelas siswa.</p>
                </div>
            </div>
